...
public class Graph {
	...
	public Graph primJarnik() {
		...
		while (result.nameToNode.size()<nameToNode.size())
		{
			...
			{
				if (!result.containsNode(n.getName())) {
			...
			Node n1 = result.getOrCreateNode(e.getSrc().getName());  
			if (!result.containsNode(e.getNd().getName()))
			{
				Node n2 = result.getOrCreateNode(e.getNd().getName());
				n1.addUndirectedEdgeToNode(n2, e.weight);
				startNode = e.getNd();
			}
	...
	public static Graph createDirectedWeightedGraphFromEdgeList(InputStream in) throws IOException {
		...
		while (scan.hasNext())
		{
			Node n1 = g.getOrCreateNode(scan.next());
			Node n2 = g.getOrCreateNode(scan.next());
			n1.addDirectedEdgeToNode(n2, scan.nextInt());
		}
		return g;
	}
