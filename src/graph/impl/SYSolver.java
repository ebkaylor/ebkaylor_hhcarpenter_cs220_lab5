...
public class SYSolver
{
    ...
    public static Graph readGraphFromFile(InputStream in) throws IOException
    {
        ...
        while (scan.hasNext())
        {
        	...
        	n1.addUndirectedEdgeToNode(n2, scan.next().charAt(0));
        }
    ...
    {
     Node firstMove = g.getOrCreateNode(start);
     ...
     for (int i = 0; i < 5; i++) {
         Set <String> names = new HashSet <String>();
	    	...
	    	for (Node n: firstMove.getNeighbors())
	        {
	    		String name = n.myName;
	    		names.add(name);
	        }
         	}
         	else
         	{
         		for (String bar : names)
         		{
         			...
         			{
         				names.add(pancake.myName);
         			}
         		}
         	}
	    	possibleMoves.put(i+1, names);
