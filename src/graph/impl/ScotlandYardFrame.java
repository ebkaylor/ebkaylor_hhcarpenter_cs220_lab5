...
public class ScotlandYardFrame extends JFrame
{
	...
	// In other words, do we want to know everywhere X could be after 3 moves,
	// or do we want to see everywhere he could be, plus everywhere he might have been?
	// This variable will be set to true and false by an ActionListener
	...
	public ScotlandYardFrame() throws IOException {
		...
		canvas=new JPanel() {
			...
			public void paint(Graphics graphics) {
				...
				Point p = pointMap.get(startNode);
				g.fillOval((int) p.getX()-10, (int) p.getY()-12, 15, 15);
				if (inclusive){
					for (int i=0; i<numMoves; i++)
					{
						g.setColor(colors.get(i));
						Set<String> bar = moves.get(i+1);
						for (String pop : bar)
						{
							Point q = pointMap.get(pop);
							g.drawOval((int) q.getX()-20-2*i,(int) q.getY()-20-2*i, 40+3*i, 40+4*i);
				...
				{
					Set<String> boppa = moves.get(numMoves);
					for (String q : boppa)
					{
						g.setColor(colors.get(numMoves-1));
						Point fooly = pointMap.get(q);
						g.drawOval((int) fooly.getX()-20,(int) fooly.getY()-20, 40, 40);
	...
	private void addMouseHandlers() {
		canvas.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				for (String pop: pointMap.keySet()) {
					double dist = clicked.distance(pointMap.get(pop));
